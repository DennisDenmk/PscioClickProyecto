        ...
        <div class="relative text-white hover:text-blue-500 dark:hover:text-[#38BDF8]">
            ...
            <div id="notificacionesPanel"
                ...
                @forelse (auth()->user()->notifications as $notification)
                    <div
                        class="p-3 border-b text-sm {{ $notification->read_at ? 'bg-white' : 'bg-blue-50 dark:bg-blue-900/20' }}">
                        ...
                        <div class="flex justify-between items-center mt-1">
                            @if (!$notification->read_at)
                                <form method="POST"
                                    ...
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="text-gray-500 text-xs hover:text-green-600">Marcar
                                        como leída</button>
                                </form>
                            @endif
                        </div>
