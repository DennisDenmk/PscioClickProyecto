<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">
            Antecedentes - Historia Clínica #{{ $historia->his_id }}
        </h2>
    </x-slot>
    ...
    <div class="py-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                <h4 class="text-lg font-semibold text-gray-800">Antecedentes Registrados</h4>
            </div>

            @if($historia->antecedentes->isEmpty())
                <div class="p-6 text-center text-gray-500">
                    No hay antecedentes registrados para esta historia clínica.
                    <div class="mt-2">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tipo de Antecedente</th>
                                ...
                                @foreach ($historia->antecedentes as $ant)
                                    <tr class="hover:bg-gray-50">
                                            ...
                                            <div class="flex items-center">
                                                ...
                                                <div class="ml-3">
                                                    <div class="text-sm font-medium text-gray-900">
                                                        {{ $ant->tipoAntecedente->tipa_nombre }}
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-900">
                                            @if ($ant->ant_detalle)
                                                <div class="max-w-xs">
                                                    {{ Str::limit($ant->ant_detalle, 60) }}
                                                    @if(strlen($ant->ant_detalle) > 60)
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {{ $ant->created_at ? $ant->created_at->format('d/m/Y') : 'N/A' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                            <button class="text-blue-600 hover:text-blue-900 font-medium"
                                                    onclick="mostrarDetalle('{{ $ant->tipoAntecedente->tipa_nombre }}', '{{ $ant->ant_detalle }}')">
                                                Ver Detalle
                                            </button>
                                        </td>
                                    </tr>
                            ...
                            <div class="bg-gray-50 rounded-lg p-4 border hidden md:block">
                                <div class="grid grid-cols-1 gap-3">
                                    <div class="flex items-center">
                                        ...
                                        <div class="font-medium text-gray-900">{{ $ant->tipoAntecedente->tipa_nombre }}</div>
                                    </div>
                                    @if ($ant->ant_detalle)
                                        <div>
                                            <div class="text-sm font-medium text-gray-500">Detalle</div>
                                            <div class="text-sm text-gray-900">{{ $ant->ant_detalle }}</div>
                                        </div>
                                    @endif
        ...
        <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-1/2 lg:w-1/3 shadow-lg rounded-md bg-white">
            <div class="mt-3">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center">
                        <div class="flex-shrink-0 h-3 w-3 bg-red-500 rounded-full mr-3"></div>
                        <h3 class="text-lg font-medium text-gray-900" id="modalTitulo">Detalle del Antecedente</h3>
                    </div>
    ...
    <script>
        function mostrarDetalle(tipoAntecedente, detalle) {
            document.getElementById('modalTitulo').textContent = tipoAntecedente;
            document.getElementById('modalContenido').textContent = detalle || 'Sin detalle específico registrado para este antecedente.';
