    ...
    <div class="py-10 max-w-7xl mx-auto px-4">
        ...
        <div class="mb-6 flex flex-wrap gap-3">
            <a href="{{ route('detalles.create', $historia->his_id) }}"
                ...
                onmouseover="this.style.backgroundColor='#1a5555'"
                onmouseout="this.style.backgroundColor='#2d7a6b'">
                + Agregar Detalle
            </a>
                        ...
                        <tbody class="divide-y" style="background-color: white; divide-color: #c8e6dc;">
                            @foreach ($historia->detallesHistoria as $detalle)
                                <tr class="hover:bg-opacity-50 transition-colors duration-150" 
                                    ...
                                    <td class="px-4 py-3 text-sm" style="color: #1a5555;">{{ $detalle->deth_fecha_valoracion }}</td>
                                    <td class="px-4 py-3 text-sm" style="color: #2d7a6b;">{{ $detalle->deth_motivo_consulta }}</td>
                                    ...
                                    <td class="px-4 py-3 text-sm">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('detalles.edit', $detalle->deth_id) }}"
                                                ...
                                                style="color: #1a5555;"
                                                onmouseover="this.style.color='#2d7a6b'"
                            ...
                            <tbody class="divide-y" style="background-color: white; divide-color: #c8e6dc;">
                                @foreach ($historia->signosVitales as $signo)
                                    <tr class="hover:bg-opacity-50 transition-colors duration-150"
                                        ...
                                        <td class="px-4 py-3 text-sm" style="color: #2d7a6b;">{{ $signo->sig_frecuencia_cardiaca }}</td>
                                        <td class="px-4 py-3 text-sm" style="color: #2d7a6b;">{{ $signo->sig_frecuencia_respiratoria }}</td>
                                        <td class="px-4 py-3 text-sm" style="color: #2d7a6b;">{{ $signo->sig_saturacion_oxigeno }}%</td>
                                        <td class="px-4 py-3 text-sm" style="color: #2d7a6b;">{{ $signo->sig_temperatura }}°C</td>
                                        <td class="px-4 py-3 text-sm">
