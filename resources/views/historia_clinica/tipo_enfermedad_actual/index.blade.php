<x-app-layout>
    ...
    <div class="py-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Botón "Nuevo Tipo" -->
        <div class="mb-6">
            <a href="{{ route('tipo_enfermedad_actual.create') }}"
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
            @if ($tipos->isEmpty())
                <div class="p-6 text-center text-gray-500">
                    No hay tipos de enfermedad actual registrados.
                </div>
            @else
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($tipos as $tipo)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $tipo->tipo_enf_id }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $tipo->tipo_enf_nombre }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                            <a href="{{ route('tipo_enfermedad_actual.edit', $tipo->tipo_enf_id) }}"
                        ...
                        @foreach ($tipos as $tipo)
                            ...
                            <div class="bg-gray-50 rounded-lg p-4 border hidden md:block">
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <div class="text-sm text-gray-900">{{ $tipo->tipo_enf_id }}</div>
                                    </div>
                                    <div>
                                        <div class="text-sm font-medium text-gray-500">Nombre</div>
                                        <div class="text-sm text-gray-900">{{ $tipo->tipo_enf_nombre }}</div>
                                    </div>
                            ...
                            <div class="bg-gray-50 rounded-lg p-3 border block md:hidden">
                                <div class="flex justify-between items-start mb-2">
                                    <div class="flex-1">
                                        <div class="font-medium text-gray-900">{{ $tipo->tipo_enf_nombre }}</div>
                                        <div class="text-sm text-gray-600">ID: {{ $tipo->tipo_enf_id }}</div>
                                    </div>
                                </div>
